...
(async function build() {
  ...
  /**
   * Get the JS files from `./dist/STKTableWP/` directory and combine it into `bundle.js`
   * This will give support for angular v6 and all above versions.
   */
 
  try {
    let directoryPath = './dist/STKTableWP/';
    let files = await getFilesFromDirByType(directoryPath, 'js');
    files = files.map(file => directoryPath + file);
    await concat(files, './dist/STKTableWP/bundle.js');
  } catch (error) {
    console.log(error);
  }
})();
